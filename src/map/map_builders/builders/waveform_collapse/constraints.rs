...
pub fn build_patterns(
    ...
    for cy in 0..chunks_y {
        for cx in 0..chunks_x {
            // Normal orientation
            let mut pattern: Vec<GameTile> = Vec::new();
...
pub fn render_pattern_to_map(map: &mut Map, chunk: &MapChunk, chunk_size: i32, start_x: i32, start_y: i32) {
    ...
    for tile_y in 0..chunk_size {
        for tile_x in 0..chunk_size {
            ...
            map.visible.set_bit(Point::new(start_x + tile_x, start_y + tile_y), true);
            i += 1;
        }
    }
    ...
    for (x, northbound) in chunk.exits[0].iter().enumerate() {
        if *northbound {
            ...
            map.tiles[map_idx] = GameTile::stairs_down();
        }
    }
...
pub fn patterns_to_constraints(patterns: Vec<Vec<GameTile>>, chunk_size: i32) -> Vec<MapChunk> {
    ...
    for p in patterns {
        ...
        for x in 0..chunk_size {
            ...
            if new_chunk.pattern[north_idx].tile_type == TileType::Floor {
            ...
            if new_chunk.pattern[south_idx].tile_type == TileType::Floor {
            ...
            if new_chunk.pattern[west_idx].tile_type == TileType::Floor {
            ...
            if new_chunk.pattern[east_idx].tile_type == TileType::Floor {
