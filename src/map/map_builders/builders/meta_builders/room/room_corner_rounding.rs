...
impl RoomCornerRounder {
    ...
    fn fill_if_corner(&mut self, x: i32, y: i32, build_data: &mut BuilderMap) {
        ...
        if x > 0 && build_data.map.tiles[idx - 1].tile_type == TileType::Wall {
            neighbor_walls += 1;
        }
        if y > 0 && build_data.map.tiles[idx - w as usize].tile_type == TileType::Wall {
            neighbor_walls += 1;
        }
        if x < w - 2 && build_data.map.tiles[idx + 1].tile_type == TileType::Wall {
            neighbor_walls += 1;
        }
        if y < h - 2 && build_data.map.tiles[idx + w as usize].tile_type == TileType::Wall {
            neighbor_walls += 1;
        }
