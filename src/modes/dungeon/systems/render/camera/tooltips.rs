...
pub fn render_tooltips(
    ...
    tooltip_q.iter().filter(|(pos, _, _, _)| pos.0 == map_pos).for_each(|(pos, name, desc, stats)| {
    ...
    if !lines.is_empty() {
        ...
        let tip_y = if map_pos.y > MAPHEIGHT as i32 / 2 { mouse_y - height as i32 } else { mouse_y };
